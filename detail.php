...
<body class="bg-cover bg-center bg-fixed">
    ...
    <div class="bg-violet-700 shadow-lg ml-4 mt-4 shadow-violet-700/60 transition group hover:bg-white hover:shadow-slate-800/60 inline-block rounded-full px-4">
        <a href="<?= $_SESSION['path'] ?><?= htmlspecialchars($_SESSION['current'] === 'null' ? '' : '?category=' . $_SESSION['current']) ?>">
    ...
    <div class="container md:flex-row flex flex-col md:items-start mx-auto w-[90%] mt-4 pb-24 gap-4">
        <div class="rounded-xl group hover:scale-95 transition duration-300 overflow-hidden w-[90%] md:w-[40%] mx-auto shadow-lg shadow-slate-800/60">
            <img class="group-hover:scale-110 group-hover:rotate-3 transition duration-300" src="" alt="" />
        </div>
        <div class="bg-white/40 md:w-[60%] backdrop-blur-md rounded-xl px-4 pt-2 pb-4 shadow-lg shadow-slate-800/60">
            <h1 class="text-2xl font-medium flex items-center gap-2 justify-center text-slate-800">Artist
                <span>
                    <svg class="w-8 h-8 mt-1 text-gray-800 transition hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
            ...
            <?php if ($result): ?>
                <button type="submit" class="flex select-none mx-auto items-center gap-2 text-lg text-white font-medium bg-violet-700 shadow-md shadow-violet-700/60 transition hover:shadow-violet-700 hover:bg-violet-500 px-5 rounded-xl py-2 tracking-wider">Rejected
            ...
            <?php else: ?>
                <button type="submit" class="flex select-none mx-auto items-center gap-2 text-lg text-white font-medium bg-violet-700 shadow-md shadow-violet-700/60 transition hover:shadow-violet-700 hover:bg-violet-500 px-5 rounded-xl py-2 tracking-wider">Favorite
